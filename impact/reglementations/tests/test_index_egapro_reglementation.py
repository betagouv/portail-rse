...
def test_default_index_egapro_reglementation():
    ...
    assert index.primary_action.url == "https://index-egapro.travail.gouv.fr/"
    assert (
        index.primary_action.title == "Calculer et déclarer mon index sur Index Egapro"
    )
...
def test_calculate_more_than_50_employees(effectif, entreprise, mock_index_egapro):
    ...
    assert (
        index.status_detail
        == "Vous êtes soumis à cette réglementation. Vous n'avez pas encore déclaré votre index sur Index Egapro."
    )
...
def test_is_index_egapro_updated_with_up_to_date_entreprise(entreprise, mocker):
    ...
    assert is_index_egapro_updated(entreprise)


def test_is_index_egapro_not_updated(entreprise, mocker):
    # Example response from https://egapro.travail.gouv.fr/api/search?q=889297453
    index_egapro_data = """{"data":[],"count":0}"""

    mocker.patch("requests.get", return_value=MockedResponse(index_egapro_data))

    assert not is_index_egapro_updated(entreprise)
