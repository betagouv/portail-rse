...
INDICATEURS_VSME_URL = "/indicateurs/vsme/{siren}"
...
def test_page_indicateurs_vsme(client, entreprise_qualifiee, alice):
    ...
    url = INDICATEURS_VSME_URL.format(siren=entreprise_qualifiee.siren)
...
def test_chargement_templates_des_categories_de_vsme(
    categorie, client, alice, rapport_vsme
):
    # vérifie que tous les templates sont bien chargés dans la page résultante
    client.force_login(alice)

    url = CATEGORIE_VSME_URL.format(vsme_id=rapport_vsme.id, categorie_id=categorie)
