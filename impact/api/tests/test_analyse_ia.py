...
def test_succès_lancement_analyse(mocker, settings):
    settings.API_ANALYSE_IA_BASE_URL = API_ANALYSE_IA_BASE_URL = (
    ...
    etat = lancement_analyse(DOCUMENT_ID, DOCUMENT_URL, VERSION_IA, CALLBACK_URL)

    assert etat == "processing"
    faked_request.assert_called_once_with(
        f"{API_ANALYSE_IA_BASE_URL}/run-task/v{VERSION_IA}",
        {
...
def test_echec_exception_provoquee_par_l_api(mocker):
    ...
    with pytest.raises(APIError) as e:
        lancement_analyse(DOCUMENT_ID, DOCUMENT_URL, VERSION_IA, CALLBACK_URL)
