...
def test_api_fonctionnelle():
    ...
    assert len(data) == 3
    assert isinstance(data["date_cloture_exercice"], date)
    assert data["tranche_chiffre_affaires"] in [
        ca for ca, _ in CaracteristiquesAnnuelles.CA_CHOICES
    ]
    assert data["tranche_chiffre_affaires_consolide"] in [
        ca for ca, _ in CaracteristiquesAnnuelles.CA_CONSOLIDE_CHOICES
    ]
...
def test_pas_de_bilan_consolide(mocker):
    ...
    mocker.patch("requests.get", return_value=MockedResponse(200, json.loads(content)))
